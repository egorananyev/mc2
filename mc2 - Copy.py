...
while len(stairs)>0:
    ...
    for thisStair in stairs:
        curRevs = np.append(curRevs, len(thisStair.reversalIntensities))
    if nStairsDone>0:
        curRevs = np.append(curRevs, np.repeat(10,nStairsDone))
    #print 'curRevs: ' + str(curRevs)
    print 'staircase progress: %.1f%' %(np.average(curRevs)*10)
